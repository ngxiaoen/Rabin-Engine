...
void L_GrabBoat::on_update(float dt)
{
    ...
    //        auto newAgent = agents->create_behavior_agent("ExampleAgent", BehaviorTreeTypes::tentacle);
    ...
    switch (myGlobalBlackBoard->get_value<int>("collidedDir"))
    {
    case 1:
    ...
    myGlobalBlackBoard->set_value("collidedNodePositions", nodes);
    myGlobalBlackBoard->set_value("grabBoat", true);
    on_success();
    display_leaf_text();
}
